...
func (s *Server) Start() {
	...
	fmt.Println("server master started at PID " + strconv.Itoa(os.Getpid()))
	...
	go func() {
		...
		fmt.Println("server worker started at PID " + strconv.Itoa(os.Getpid()) + " listening on " + net.JoinHostPort(serverCfg.IP, serverCfg.Port))
